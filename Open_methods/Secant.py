...
class SC:
    ...
    def draw_plot(self, iterations, value_diff):
        ...
        xp = iteration['previous_approximate']
        xi = iteration['current_approximate']
        xr = iteration['approximate_root']
        diff = max(xi, xr) - min(xi, xr)
        diff = max(xi, xp, xr) - min(xi, xp, xr)
        start = min(xi, xp, xr) - 0.25 * 10 * diff
        end = max(xi, xp, xr) + 0.25 * 10 * diff
        if diff == 0:
            start = -10
            end = 10
        ...
        plt.plot([xp, xp], [0, value_diff(xp)], color='m', label='Xi-1')
...
number_of_iterations, execution_time, iterations, xr, ea, value_diff = obj.solve()
# Result = obj.solve()
# print(Result[0])
# print(Result[1])
# print(*Result[2], sep=", ")
